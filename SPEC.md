...
B4|2026-08-04|`guardrails / check` failed because `flake-manifest-check` rejects a top-level `let` in the flake outputs body and reported its helper bindings as illegal attributes|Wrap the existing outputs attrset in `nixpkgs.lib.fix`, preserving the same outputs while making the manifest structure explicit
