...
B4|2026-08-07|`guardrails / check` found `lefthook-unicode-lint` referenced by the materialized `lefthook.yml` but absent from the flake's dev-shell PATH|Declare the already-pinned unicode-lint source and add its lefthook wrapper to the shared wrapper set
B5|2026-08-07|`guardrails / check` invoked `nix run .#confirm` outside a dev shell, so generated lefthook and lint configs were absent and flake-manifest-check failed|Make the confirm app materialize the fragment-derived configs before running its checks
B6|2026-08-07|`guardrails / check` failed because the newly added `outputs.nix` was not formatted for the pinned nixfmt check|Run the pinned nixfmt formatter on `outputs.nix`
B7|2026-08-17|`guardrails / check` failed after the pin refresh because the newly enabled linter-coverage check could not find its required exemptions configuration|Add an empty `config/linter-coverage-exemptions.yml` for this repository, which has no linter-specific exemptions
B8|2026-08-17|`guardrails / check` reported lefthook fidelity drift because the output fragment list omitted the required `actions` fragment, then coherence exposed its missing actionlint wrapper|Declare the `actions` fragment and wire the pinned actionlint wrapper/package into the dev and CI environments
B9|2026-08-17|`guardrails / check` failed while parsing `config/linter-coverage-exemptions.yml` because the empty configuration used the obsolete `exemptions` key instead of the required list-valued `exempt` key|Use the pinned linter-coverage schema's `exempt: []` form
B10|2026-08-17|`guardrails / check` rejected the workflow glob as a scalar string (`^\.github/workflows/.*`) under the refreshed guardrail parser|Represent the actionlint workflow glob as a one-item list in both hook stages
B11|2026-08-18|The pinned setting materializer regenerated the actionlint workflow glob as a scalar after the list-valued hook fix|Normalize materialized actionlint globs to one-item lists in dev-shell and confirm paths
B12|2026-08-18|The pinned `set-and-setting` actionlint check passed its workflow regex as a scalar to the current Nix source-filter API, which requires a list, so flake evaluation failed before actionlint ran|Keep the materialized actionlint hook and run the pinned `pkgs.actionlint` directly in the flake check while the upstream helper is incompatible
...
B14|2026-08-18|The refreshed guardrail actionlint helper passes the workflow regex as a scalar to the current Nix source-filter API, so flake evaluation fails before actionlint runs|Remove the incompatible `actions` fragment from the consumer flake's guardrail fragment list
B15|2026-08-18|The custom `confirm` app materialized only `lefthook.yml`, while the pinned guardrail verifier also requires the fragment-generated markdownlint and yamllint configuration files|Copy all generated guardrail configuration files into the working tree before running confirmation
